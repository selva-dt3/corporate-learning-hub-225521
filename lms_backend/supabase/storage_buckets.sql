...
-- APPLY THIRD (after policies.sql)
-- Idempotency:
--   - Bucket creation uses ON CONFLICT DO NOTHING (safe to re-run).
--   - Object policies drop before create (safe to re-run).
-- Expectations:
--   - Buckets are private; only admin/hr manage objects directly.
--   - Employees must access objects via signed URLs generated by backend.
--   - No public buckets are created here.
-- Alignment: Backend stores content_url like 'lesson-content/...', to be served via signed URLs.
...
-- Defensive policy blocks employee direct read (select) to ensure signed URL path only.
